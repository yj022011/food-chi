...
def train():
    ...
    for i, (trainset_cv, testset_cv) in enumerate(kf.split(data)):
        ...
        algo_svd.fit(trainset_cv)
        print('SVD On trainset,', end=' ')
        predictions_svd = algo_svd.test(trainset_cv.build_testset())
        rmse(predictions_svd, verbose=True)
        print('SVD On testset,', end='  ')
        predictions_svd = algo_svd.test(testset_cv)
        rmse(predictions_svd, verbose=True)
        dump.dump('models/dump_SVD', predictions_svd, algo_svd)
